...
object KyoQueryableTree:
    ...
    private def visitChildren[T, S](children: List[T])(f: T => Unit < S)(using q: QueryableTree[T], frame: Frame): Unit < S =
        children match
            case Nil       => ()
            case h :: rest => f(h).flatMap(_ => visitChildren(rest)(f))

    private def foldChildren[T, A, S](children: List[T], acc: A)(f: (A, T) => A < S)(using q: QueryableTree[T], frame: Frame): A < S =
        children match
            case Nil       => acc
