...
/** Monaco language descriptor for the Elm dialect Krueger parses.
  ...
  * editor highlighting. [[ElmLanguageSpec]] pins that equality as a test-time contract — any drift fails CI rather
  * than shipping mismatched highlights.
  *
...
object ElmLanguage:
    ...
    /** Idempotently register Elm with Monaco. Safe to call twice — the second call is a no-op because Monaco's
      ...
      *
      * @note
      *   Must only be called in a browser context. Unit tests must not invoke this method; they assert on the pure
      *   data above.
      */
    def register(): Unit =
